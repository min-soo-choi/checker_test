...
## 🧭 사용법 (최근 변경)
- **시트 조회**: 상단 “📄 시트에서 불러오기”에서 탭(모의고사/교과서) 선택 → 작가명/작품명/지문 텍스트로 검색 → 결과 선택/버튼으로 OCR 입력에 자동 반영.
- **미리보기**: 기본 접힘 상태. 필요할 때 열어 확인.
- **복사 편의**: 최종본/편집본 옆 복사 버튼으로 클릭 복사(clipboard API→execCommand 폴백).
- **편집 동기화**: 실행 시 편집/최종본이 최신 결과로 자동 갱신. 버튼으로 편집본→최종본 반영 가능.
...
## 🧭 사용법 (최근 변경)
- **최종본 복사**: components 기반 복사 버튼으로 최종본 텍스트를 안전하게 복사(clipboard API→execCommand 폴백).
- **미리보기**: 기본 접힘 상태. 필요할 때 열어 확인.
- **라벨 옵션(7번)**: 7번 기능 라벨 UX를 3번과 동일하게(대문자/소문자 선택) 자동 부여.
- **편집 동기화**: 실행 시 편집/최종본이 최신 결과로 자동 갱신. 버튼으로 편집본→최종본 반영 가능.
...
- **시트 조회**: 국어 작업에서 탭(모의고사/교과서) 선택 후 작가명·작품명·지문 텍스트로 검색 → 결과 선택 시 입력에 자동 반영.
- **미리보기 토글**: ko_work/en_work 미리보기는 기본 접힘 상태. 필요할 때만 열어 확인.
- **복사 편의**: 최종본/편집본에 복사 버튼 추가(clipboard API→execCommand 폴백). 영어 작업은 최종본 텍스트를 components 기반으로 안정적으로 복사.
- **라벨 옵션(7번)**: en_work 7번 기능 라벨 UX를 3번과 동일하게 (대문자/소문자 선택) 자동 부여로 통일.
- **편집 동기화**: 실행 시 편집 영역/최종본이 최신 결과로 자동 갱신되며, 버튼으로 편집본→최종본 반영 가능.
"""
    }
